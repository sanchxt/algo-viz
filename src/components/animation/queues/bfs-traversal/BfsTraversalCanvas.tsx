...
interface BfsTraversalCanvasProps {
...
const MemoizedQueueElement = memo(
  ({
    element,
    nodeValue,
    styleClasses,
    ...
    <motion.div
      key={element.id}
      layoutId={`queue-elem-${element.id}`}
      className={`absolute flex flex-col items-center justify-center rounded-lg border text-xs font-mono shadow-lg ${styleClasses}`}
      initial={{ opacity: 0, scale: 0.5 }}
      animate={{
        left: position.x,
        top: position.y,
    ...
    >
      <div className="font-semibold">{nodeValue}</div>
      <div className="text-xs opacity-75">L{element.level}</div>
    </motion.div>
  )
);
...
const BfsTraversalCanvas = ({ currentStep }: BfsTraversalCanvasProps) => {
  ...
  const queueData: QueueElement[] =
    currentStep.dataStructures.queue?.data || [];
  ...
  // queue layout - horizontal arrangement
  const queueLayout = useMemo(() => {
    const positions = new Map<string, { x: number; y: number }>();
    const elementWidth = 70; // 60px element + 10px gap
    const startX = 20;
    const centerY = QUEUE_HEIGHT / 2 - 25; // center vertically in queue area

    queueData.forEach((element, index) => {
      positions.set(element.id, {
        x: startX + index * elementWidth,
        y: centerY,
      });
    });
  ...
  const highlights = useMemo(() => {
    ...
    const queueHighlights = currentStep.highlights.queue || [];
    ...
    return {
      tree: {
        ...
        highlight: getHighlightStyle(treeHighlights, "highlight"),
        match: getHighlightStyle(treeHighlights, "match"),
        level_complete: getHighlightStyle(treeHighlights, "level_complete"),
      },
      queue: {
        highlight: getHighlightStyle(queueHighlights, "highlight"),
        queued: getHighlightStyle(queueHighlights, "queued"),
        dequeued: getHighlightStyle(queueHighlights, "dequeued"),
      },
      result: {
  ...
  const getNodeStyle = useCallback(
    (nodeId: string) => {
      ...
      if (highlights.tree.highlight.includes(nodeId)) {
      ...
      if (highlights.tree.level_complete.includes(nodeId)) {
      ...
      if (highlights.tree.match.includes(nodeId)) {
        return "from-cyan-400 via-cyan-500 to-cyan-600 border-cyan-300/70 ring-2 ring-cyan-300/40";
      }
  ...
  const getQueueElementStyle = useCallback(
    (elementId: string) => {
      if (highlights.queue.highlight.includes(elementId)) {
        return "bg-blue-500/30 border-blue-400/60 text-blue-200 ring-2 ring-blue-400/40";
      }
      if (highlights.queue.queued.includes(elementId)) {
        return "bg-emerald-500/30 border-emerald-400/60 text-emerald-200";
      }
      if (highlights.queue.dequeued.includes(elementId)) {
        return "bg-red-500/30 border-red-400/60 text-red-200";
      }
      return "bg-white/10 border-white/30 text-gray-200";
    },
    [highlights.queue]
  );
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <div className="w-full max-w-full overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
        <div
          ref={containerRef}
          className="mx-auto flex w-full max-w-7xl flex-col rounded-xl border border-white/10 bg-gray-900/30 p-4 gap-y-4"
          style={{
          ...
          <div className="w-full">
            <div className="rounded-lg bg-black/10 p-3">
              ...
              <AnimatePresence>
                {!isQueueCollapsed && (
                  <motion.div
                    className="relative bg-white/5 border border-white/20 rounded-lg overflow-x-auto"
                    style={{ height: QUEUE_HEIGHT, minWidth: "100%" }}
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: QUEUE_HEIGHT }}
                  ...
                  >
                    {queueData.length === 0 ? (
                      ...
                      <>
                        {/* front/rear labels */}
                        <div className="absolute top-2 left-2 text-xs text-emerald-400 font-semibold">
                          FRONT
                        </div>
                        <div className="absolute top-2 right-2 text-xs text-purple-400 font-semibold">
                          REAR
                        </div>

                        {/* queue elements */}
                        <AnimatePresence>
                          {queueData.map((element, index) => {
                            const position = queueLayout.get(element.id);
                            if (!position) return null;

                            const node = treeData.nodes.find(
                              (n) => n.id === element.nodeId
                            );
                            if (!node) return null;

                            const styleClasses = getQueueElementStyle(
                              element.id
                            );
                            const isScaled =
                              highlights.queue.highlight.includes(element.id);

                            return (
                              <MemoizedQueueElement
                                key={element.id}
                                element={element}
...
export default BfsTraversalCanvas;
